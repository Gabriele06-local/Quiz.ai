...
Hint: pensa a browser e API REST.`,
  ...
  function buildHistoryQuizRow(entry, folderOpts, folderGroups) {
    ...
    btnRen.addEventListener("click", async () => {
      const n = promptRename(
        "Nuovo titolo (solo etichetta, non modifica il testo del quiz):",
        entry.title,
      );
  ...
  function onQuizKeyboardShortcut(e) {
    ...
    if (e.key === "Enter") {
      ...
      if (
        studyFormat === "fill_open" &&
        t instanceof HTMLInputElement &&
        t.id === "fill-open-input"
      ) {
        e.preventDefault();
  ...
  function renderFillBank() {
    ...
    keys.forEach((k) => {
      ...
      btn.addEventListener("click", () => {
        ...
        chips
          .querySelectorAll(".study-chip")
          .forEach((c) => c.classList.remove("study-chip--selected"));
    ...
    if (!answered) {
      ...
      if (chips) {
        chips.querySelectorAll(".study-chip").forEach((b) => {
          ...
          if (!examMode && b.dataset.key === fillBankSelectedKey && !ok)
            b.classList.add("study-chip--wrong");
        });
      }
      if (cbtn) {
        cbtn.disabled = false;
  ...
  function onMatchPickLeft(si) {
    ...
    matchPickLeftSi = si;
    $("match-col-left")
      ?.querySelectorAll(".match-tile-left")
      .forEach((b) => {
  ...
  function onMatchPickRight(siR) {
    ...
    if (siL === siR) {
      ...
      $("match-col-right")
        ?.querySelectorAll(".match-tile-right")
        .forEach((b) => {
      ...
      $("match-col-left")
        ?.querySelectorAll(".match-tile-left")
        .forEach((b) => b.classList.remove("match-tile--picked"));
      if (fb) {
        fb.hidden = false;
  ...
  function argomentazioneFeedback(ok, argText) {
    ...
    if (!t) return "";
    if (t.length < 15)
      return "Hai aggiunto un'argomentazione breve: va bene per un ripasso veloce.";
  ...
  Q.initQuizApp = function initQuizApp() {
    ...
    function wireExportAll(id, fmt) {
      $(id)?.addEventListener("click", async () => {
        ...
        if (!ok)
          showHistorySaveError("Accedi e assicurati che Supabase sia configurato per esportare.");
